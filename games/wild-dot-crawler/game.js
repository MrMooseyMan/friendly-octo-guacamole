document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadMeta() {
        ...
        return { totalRuns: 0, bestFloor: 0, totalEnemiesKilled: 0, totalBossesKilled: 0, totalGoldEarned: 0, dragonKingDefeated: false, achievements: [] };
    }
    ...
    let state = {
        ...
        player: {
            ...
            special: 0, maxSpecial: 10, dmgBonus: 0, specialRechargeBonus: 0,
            gold: 0, potions: 1, critChance: 0.1,
            bleed: 0, burn: 0, cursed: false,
            upgrades: [],
            thorns: 0, lifestealPct: 0, goldFindBonus: 0, surgeDmgBonus: 0,
            dmgReduction: 0, chainBonusPct: 0, regenPerTurn: 0, shieldBonus: 0,
        },
        enemy: null,
        stats: { enemiesDefeated: 0, bossesDefeated: 0, damageDealt: 0, damageReceived: 0, goldEarned: 0, criticalHits: 0, longestChain: 0, potionsUsed: 0, cascades: 0 },
        audioReady: false
    };
    ...
    function spawnEnemy() {
        ...
        state.enemy = {
            ...
            isBoss: template.isBoss || false, enraged: false,
    ...
    function ensureBoardDiversity() {
        ...
        RUNE_TYPES.forEach(type => {
            while (counts[type] > 12) {
                ...
                outer: for (let x = 0; x < GRID_SIZE; x++) for (let y = 0; y < GRID_SIZE; y++) {
                    if (state.grid[x][y] && state.grid[x][y].type === type) { state.grid[x][y].type = minType; counts[type]--; counts[minType]++; break outer; }
                }
            }
        });
    }
    ...
    let cellElements = []; // flat array of 36 cell divs, created once
    ...
    function renderBoard(fullRebuild = false) {
        ...
        for (let y = 0; y < GRID_SIZE; y++) {
            for (let x = 0; x < GRID_SIZE; x++) {
                ...
                const newType = rune.type;
                if (cell._type !== newType) {
                    cell.innerHTML = ICONS[newType];
                    cell._type = newType;
                }

                // Build class
                let cls = 'rune rune-' + newType;
    ...
    // Lightweight update: only toggle selection classes, no DOM rebuild
    function updateSelectionVisuals() {
        for (let y = 0; y < GRID_SIZE; y++) {
            for (let x = 0; x < GRID_SIZE; x++) {
                const cell = cellElements[y * GRID_SIZE + x];
                if (!cell) continue;
                const isSelected = state.selected.some(s => s.x === x && s.y === y);
    ...
    // --- Position-based cell detection (replaces elementFromPoint) ---
    ...
    function processMatch() {
        ...
        const counts = {};
        state.selected.forEach(p => { const t = state.grid[p.x][p.y].type; if (t !== 'white') counts[t] = (counts[t] || 0) + 1; });
        ...
        // Check cascades then finish move
        setTimeout(() => {
            checkCascades(0, () => {
                state.moves--;
    ...
    function applyMatchEffect(type, power, comboMult, isCrit, cells) {
        ...
        switch (type) {
            ...
            case 'blue': {
                ...
                chargeSpecial(charge); msg = `+${charge} Surge`; color = "#0ea5e9"; playSound('powerup'); break;
            }
            case 'yellow': {
    ...
    // --- Cascade (line-based: horizontal/vertical runs of 3+) ---
    ...
    function checkCascades(depth, onComplete) {
        ...
        // Don't cascade if enemy already dead
    ...
    function chargeSpecial(amount) { state.player.special = Math.min(state.player.special + amount, state.player.maxSpecial); updateUI(); }
    ...
    function activateSurge() {
        ...
        state.player.special = 0; playSound('surge');
        let surgeDmg = 15 + state.player.level * 3;
        if (state.player.surgeDmgBonus > 0) surgeDmg = Math.floor(surgeDmg * (1 + state.player.surgeDmgBonus));
        damageEnemy(surgeDmg); healPlayer(8); addShield(8);
        screenShake(true);
    ...
    function enemyDefeated() {
        ...
        let potionDrop = 0;
        if (Math.random() < (state.enemy.isBoss ? 0.8 : 0.2)) { potionDrop = 1; state.player.potions++; }
    ...
    function nextBattle() {
        ...
        state.floor++; state.moves = getMovesPerTurn(); state.turn = 1; state.combo = 0;
        healPlayer(Math.floor(state.player.maxHp * 0.1));
        state.player.bleed = 0; state.player.burn = 0;
        spawnEnemy(); generateGrid(true); renderBoard(true); updateUI();
    }
    ...
    function restartGame() {
        ...
        state.combo = 0; state.processing = false;
        state.stats = { enemiesDefeated: 0, bossesDefeated: 0, damageDealt: 0, damageReceived: 0, goldEarned: 0, criticalHits: 0, longestChain: 0, potionsUsed: 0, cascades: 0 };
        meta = loadMeta();
        applyMetaPerks(state.player);
        state.moves = getMovesPerTurn();
        spawnEnemy(); generateGrid(true); renderBoard(true); updateUI(); showActivePerks();
    }
    ...
    function generateUpgradeChoices() {
        ...
        const colorMap = {
            ...
            sky: ['rgba(12,74,110,0.5)', 'rgba(14,165,233,0.5)', '#0ea5e9', 'rgba(12,74,110,0.8)'],
    ...
    function applyUpgrade(type) {
        ...
        switch (type) {
            ...
            case 'special': state.player.specialRechargeBonus += 2; state.player.maxSpecial = Math.max(5, state.player.maxSpecial - 2); break;
            ...
            case 'surgedmg': state.player.surgeDmgBonus += 0.5; break;
    ...
    function endTurn() {
        ...
        setTimeout(() => {
            ...
            for (let i = 0; i < atkCount; i++) {
                let dmg = atkCount > 1 ? Math.floor(state.enemy.damage / 2) : state.enemy.damage;
                const pierces = state.enemy.ability === 'pierce';
                if (!pierces && state.player.shield > 0) {
                    if (state.player.shield >= dmg) { state.player.shield -= dmg; dmg = 0; } else { dmg -= state.player.shield; state.player.shield = 0; }
                }
                if (state.player.dmgReduction > 0 && dmg > 0) dmg = Math.max(1, Math.floor(dmg * (1 - state.player.dmgReduction)));
                totalDmg += dmg;
            }
            ...
            if (totalDmg > 0) {
                ...
                if (state.player.thorns > 0 && state.enemy.hp > 0) {
                    state.enemy.hp -= state.player.thorns; state.stats.damageDealt += state.player.thorns;
            ...
            if (state.enemy.ability === 'drain') { const d = Math.floor(state.player.special * 0.3); state.player.special -= d; if (d > 0) spawnFloatingText(elPlayerPortrait, `-${d} Surge`, "#0ea5e9"); }
    ...
    function updateUI() {
        ...
        btnSurge.classList.toggle('hidden', state.player.special < state.player.maxSpecial);
        if (btnPotion) { const show = state.player.potions > 0; btnPotion.classList.toggle('hidden', !show); btnPotion.disabled = !show; }
        if (elFloorDisplay) {
            const isBoss = state.floor % BOSS_EVERY === 0;
            elFloorDisplay.innerHTML = isBoss ? `<span class="text-amber-400">\u{2694}\u{FE0F} BOSS FLOOR ${state.floor}</span>` : `Floor ${state.floor}`;
        }
        updateEnemyUI();
